...
/**
 * 媒体数据报 25B 大端定长头编解码（独立 UDP 端口承载，非 protobuf）。
 *
...
final class MediaDatagram {
    ...
    /**
     * 编码一个媒体数据报：25B 头 + 负载切片 {@code payload[off, off+len)}。
     */
    static byte[] encode(int clientId, int msgId, int seq, int frag, int fragTotal,
                         long pts, int flags, byte[] payload, int off, int len) {
    ...
    /**
     * 解析数据报头中的 client_id；数据报长度不足 25B（头不合法）返回 -1。
     *

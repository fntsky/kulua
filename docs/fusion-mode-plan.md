...
>
> ⚠️ **变更（2026-08-16）**：虚拟显示器分辨率跟随窗口物理尺寸（每 500ms 轮询
> 窗口尺寸发 RESIZE_DISPLAY，替代不稳定的 Resized 事件），并新增 `--scale <系数>`
> 参数（默认 1.0，范围 0.1~4.0）：视频分辨率 = 窗口物理尺寸 × 系数，
> 系数 <1 降低编码分辨率换流畅度/带宽，>1 超采样更清晰。
>
> ⚠️ **变更（2026-08-16）**：resize 发送加**防抖**——尺寸须连续稳定 1s（2 轮 ×
> 500ms）才发 RESIZE_DISPLAY。拖动窗口边缘时内尺寸连续变化，若每轮都发，
> server 每次重建 MediaCodec（几百 ms 断流）+ 客户端重建解码器，画面闪烁/花屏
> （实测"分辨率不能稳定变换"）；防抖后拖动中一次不发、松手只发最终尺寸一次，
> 发送失败不记录 last_sent 以便下轮重试。

                    ...
                    Some(Ok((FRAME_TYPE_EVENT, data))) => {
                        if let Ok(event) = proto::Event::decode(&data[..]) {
                            ...
                            match event.event.as_str() {
                                "device.updated" => {
                                    if let Ok(devices) =
                                        serde_json::from_value::<Vec<sync_core::types::Device>>(val)
                                    {
                                ...
                                "session.updated" => {
                                    if let Ok(session_data) = serde_json::from_value::<sync_core::ipc::types::SessionListData>(val) {
                                        let _ = app.emit("sessions-updated", &session_data);
                                    }
                                }
                                "clipboard.changed" => {
                                    let _ = app.emit("clipboard-changed", &val);
                                }
                                "notification" => {
                                    let _ = app.emit("notification-received", &val);
                                }
                                _ => {}
                            }
                        }
                        ...
                        if let Ok(resp) = proto::Response::decode(&data[..]) {
                            if let Some(tx) = pending.remove(&resp.id) {
                                let result = if !resp.result.is_empty() {
                                    ...
                                    Err(format!("{} (code {})", e.message, e.code))
                                } else {
                                    Ok(serde_json::Value::Null)
                                };
                                let _ = tx.send(result);
                            } else if !resp.result.is_empty() {
                                // 初始握手响应（id=1 或 id=2）
                                if let Ok(result) = serde_json::from_slice::<serde_json::Value>(&resp.result) {
                                    match resp.id {
                                        1 => {
                                            if let Ok(devices) =
